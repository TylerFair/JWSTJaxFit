...
def _extract_harmonica_limb_samples(rors_samples, a1_samples, planet_index=0):
    a0 = np.asarray(rors_samples)
    a1 = np.asarray(a1_samples)
    ...
    if a1.ndim == 1:
        ...
        raise ValueError(f"Unsupported a1 sample shape: {a1.shape}")

    if a1.shape[1] == 1 and a0.shape[1] > 1:
        a1 = np.repeat(a1, a0.shape[1], axis=1)

    if a0.shape != a1.shape:
        raise ValueError(
            f"rors and a1 sample shapes must match after broadcasting, got {a0.shape} and {a1.shape}."
        )

    return a0, a1

def build_harmonica_limb_dataframe(wavelengths, wavelength_err, rors_samples, a1_samples,
                                   bandpass_min=None, bandpass_max=None, planet_index=0):
    wl_arr, wl_err_arr = _coerce_wavelength_axis(wavelengths, wavelength_err)
    a0_samp, a1_samp = _extract_harmonica_limb_samples(
        rors_samples, a1_samples, planet_index=planet_index
    )
    ...
    r_evening_samp = a0_samp + a1_samp
    r_morning_samp = a0_samp - a1_samp
    ...
    a1_med = np.nanpercentile(a1_samp, 50, axis=0)
    a1_lo = a1_med - np.nanpercentile(a1_samp, 16, axis=0)
    a1_hi = np.nanpercentile(a1_samp, 84, axis=0) - a1_med
    ...
    return limb_df, a0_samp, a1_samp
...
def save_harmonica_limb_products(
    ...
    limb_df, a0_samp, a1_samp = build_harmonica_limb_dataframe(
        wavelengths,
        wavelength_err,
        rors_samples,
        a1_samples,
    ...
    a1_med = limb_df["a1_median"].values
    ...
    if wl_arr.size > 1:
        ...
        for i in range(len(wl_arr)):
            r_i = np.array([a0_med[i], a1_med[i], 0.0])
    ...
    if posterior_strings_path is not None:
        ...
        ax.set_aspect("equal", "datalim")
        for j in draw_idx:
            r_sample = np.array([a0_samp[j, i_show], a1_samp[j, i_show], 0.0])
        ...
        r_med = np.array([a0_med[i_show], a1_med[i_show], 0.0])
...
def main():
    ...
    if not stringcheck or ('gp' in detrending_type):
        if not os.path.exists(f'{output_dir}/{instrument_full_str}_whitelight_GP_database.csv'):
            ...
            whitelight_model_for_run = create_whitelight_model(detrend_type=detrending_type, n_planets=n_planets, ld_profile=ld_profile, transit_engine=transit_engine)
            
            if 'gp' in detrending_type:
                print("--- Running Pre-Fit with Linear Detrending to stabilize GP ---")
                whitelight_model_prefit = create_whitelight_model(detrend_type='linear', n_planets=n_planets, ld_profile=ld_profile, transit_engine=transit_engine)
            ...
            def _soln_to_physical_params(soln, base_params, n_planets=1):
                ...
                if transit_engine == 'harmonica' and "a_rs" in p and "b" in p and "rors" in p:
            ...
            try:
                ...
                if transit_engine == 'harmonica':
                    params_complete["a_rs"] = _ensure_len(
                        params_complete.get("a_rs", HARMONICA_A_RS), n_planets_sanity
                    )
                ...
                stage1 = optimx.optimize(
                    ...
                    sites=(
                        ...
                        if (transit_engine == 'harmonica' and n_planets_sanity == 1)
            ...
            if transit_engine == 'harmonica' and 'a1' in wl_samples:
    ...
    if need_lowres_analysis and valid is None:
        ...
        if transit_engine == 'harmonica' and 'a1' in samples_lr:
            save_harmonica_limb_products(
                wavelengths=wl_lr,
                wavelength_err=data.wavelengths_err_lr,
                rors_samples=samples_lr["rors"],
                a1_samples=samples_lr["a1"],
    ...
    if transit_engine == 'harmonica' and 'a1' in samples_hr:
        save_harmonica_limb_products(
            wavelengths=wl_hr,
            wavelength_err=data.wavelengths_err_hr,
            rors_samples=samples_hr["rors"],
            a1_samples=samples_hr["a1"],
